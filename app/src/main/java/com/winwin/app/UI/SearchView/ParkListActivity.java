...
import com.winwin.app.MVP.Presenter.ParkListPresent;
...
public class ParkListActivity extends AppCompatActivity implements BaseQuickAdapter.RequestLoadMoreListener, SpringView.OnFreshListener, ParkListView {
    ...
    private ParkListPresent present;
...
//        present = new BookListPresent(this);
        ...
        if (type == 0) {
            ...
            map.clear();
            map.put("areaId", getIntent().getExtras().getInt("areaId"));
        } else if (type == 2) {
            map.clear();
            map.put("searchValue", getIntent().getExtras().getString("searchValue"));
        }
