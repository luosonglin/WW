...
import com.winwin.android.MVP.Presenter.SelectedParkListPresent;
import com.winwin.android.MVP.View.ParkListView;
import com.winwin.android.R;
import com.winwin.android.UI.Adapter.SelectParkListAdapter;
...
public class SelectedParkActivity extends AppCompatActivity implements BaseQuickAdapter.RequestLoadMoreListener, SpringView.OnFreshListener, ParkListView {
    ...
    private SelectedParkListPresent present;
    private Map<String, Object> map = new HashMap<>();
    private int parkSecondType = 0;
...
//        springView.setFooter(new RotationFooter(this)); //mRecyclerView内部集成的自动加载  上啦加载用不上   在其他View使用
        ...
        mRecyclerView.addItemDecoration(new DividerItemDecoration(SelectedParkActivity.this, DividerItemDecoration.VERTICAL));
...
//        mQuickAdapter = new ListViewAdapter(R.layout.list_view_item_layout,null);
        mQuickAdapter = new SelectParkListAdapter(R.layout.item_park, null);
        //设置加载动画
...
//        present = new BookListPresent(this);
        present = new SelectedParkListPresent(this);
        //请求网络数据
//        present.LoadData("1",PageIndex,false);
        parkSecondType = getIntent().getExtras().getInt("parkSecondType");
        map.clear();
        map.put("parkSecondType", parkSecondType);
        present.LoadData(false, map);
    }
